...
export default function CommissionButtons({ categories, works }: { categories: any[], works: any }) {
        ...
        {categories.map((cat) => {
          ...
          return (
            <Link key={cat.id} href={`/commission/${cat.id}`} className="block w-36 sm:w-44">
              <div className="relative w-full aspect-[1/1.8] rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105 group">
                {bgImage ? (
                  <div
